...
local DataLoader
local Trainer
if opt.dataset == 'imgSeg' or opt.dataset == 'lane' then
   DataLoader = require 'dataloader'
   Trainer = require 'train'
elseif opt.dataset == 'laneE' then
   DataLoader = require 'dataloaderE'
   Trainer = require 'trainMRF'
else
   DataLoader = require 'dataloader_reg'
   Trainer = require 'trainMRF'
end
...
for epoch = startEpoch, opt.nEpochs do
   ...
   if valLoss < bestLoss then
      bestModel = true
      bestLoss = valLoss
      if opt.labelType == 'seg' then
         print(string.format(' * BestResults: Err: %.3f Acc: %.3f Rec: %.3f IOU: %.3f', valLoss, Acc, Rec, IOU))
	   else
         print(string.format(' * BestResults: Err: %.3f', valLoss))
      end
   else
      if opt.labelType == 'seg' then
		   print(string.format(' * Results: Err: %.3f Acc: %.3f Rec: %.3f IOU: %.3f', valLoss, Acc, Rec, IOU))
		else
         print(string.format(' * Results: Err: %.3f', valLoss))
      end
   end
